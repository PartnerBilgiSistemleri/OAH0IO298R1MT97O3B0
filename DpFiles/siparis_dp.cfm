...
var e=wrk_query("SELECT * FROM SVK_TALEP WHERE ACTION_ID="+fatid+" AND FROM_ORDER=1","dsn3")
if(e.recordcount>0){
    $(elem[0].children).append("<li class='dropdown' id='transformation'><a style='color:#29c704' title='Sevkiyat Talebi Verildi' onclick='pencereac(2,"+fatid+")'><i class='icon-exchange'></i></a></li>")
    $(elem[0].children).append("<li class='dropdown' id='transformation'><a style='color:#b72b2b' title='Merkezden Talep Et' onclick='pencereac(3,"+fatid+")'><i class='icn-md icon-asterisk'></i></a></li>")
}else{
...
function pencereac(tip,idd){
    if(tip==1){
    windowopen('index.cfm?fuseaction=sales.emptypopup_add_sevk_talep&FROM_ORDER=1&tip=1&ACTION_ID='+idd,'wide');}else if(tip==2){
        windowopen('index.cfm?fuseaction=sales.emptypopup_add_sevk_talep&tip=2&from_order=1&ACTION_ID='+idd,'wide');
    }else if(tip==3){
        windowopen('index.cfm?fuseaction=sales.emptypopup_add_sevk_talep&tip=3&ACTION_ID='+idd,'wide');
    }else if(tip==4){
